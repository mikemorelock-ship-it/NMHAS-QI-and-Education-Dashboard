/**
 * Standalone seed script for standardized metrics.
...
// ---------------------------------------------------------------------------
async function main() {
  console.log(`Total metrics to process: ${metrics.length}`);
  ...
  const clinicalCatResult = await db.execute({
    sql: "SELECT id FROM MetricCategory WHERE slug = ?",
  ...
  for (const m of metrics) {
    // Check for existing metric by unique constraint (departmentId, slug)
    const existing = await db.execute({
      sql: "SELECT id FROM MetricDefinition WHERE departmentId = ? AND slug = ?",
      args: [departmentId, m.slug],
    });
    ...
    const categoryId = m.cat === "clinical" ? clinicalCategoryId : safetyCategoryId;
