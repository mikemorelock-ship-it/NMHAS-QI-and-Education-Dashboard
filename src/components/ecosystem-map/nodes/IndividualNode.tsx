...
const config = NODE_TYPES.individual;

function IndividualNodeComponent({ data, selected }: NodeProps) {
  const nodeData = data as { label: string; description?: string };
  return (
    <div className="relative" style={{ minWidth: 120 }}>
      <div
        className="rounded-full px-4 py-3 shadow-sm border-2 transition-shadow text-center"
        style={{
...
export const IndividualNode = memo(IndividualNodeComponent);
