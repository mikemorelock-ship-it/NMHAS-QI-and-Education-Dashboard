...
export default async function QualityImprovementPage() {
  const [campaigns, standaloneDiagrams, standaloneCycles, cycleStats, actionItemStats] =
    await Promise.all([
      ...
      prisma.pdsaCycle.findMany({
        where: {
          ...
          OR: [{ driverDiagramId: null }, { driverDiagram: { campaignId: null } }],
        },
        take: 6,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-nmh-gray">Quality Improvement</h1>
        <p className="text-muted-foreground mt-1">
          Campaigns, driver diagrams, and PDSA cycles supporting continuous improvement across NMH
          EMS operations.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-nmh-teal">{campaigns.length}</div>
            <p className="text-xs text-muted-foreground mt-1">Active Campaigns</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-nmh-orange">{totalActiveCycles}</div>
            <p className="text-xs text-muted-foreground mt-1">Active PDSA Cycles</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-nmh-gray">{totalCompletedCycles}</div>
            <p className="text-xs text-muted-foreground mt-1">Completed Cycles</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-nmh-teal">{openActionItems}</div>
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-semibold text-nmh-gray">QI Campaigns</h2>
        </div>
      ...
      {(standaloneDiagrams.length > 0 || standaloneCycles.length > 0) && (
        <section>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-semibold text-nmh-gray">Other QI Activities</h2>
          </div>
          ...
          {standaloneCycles.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <Link
                  href="/quality-improvement/pdsa"
                  className="text-sm text-nmh-teal hover:underline flex items-center gap-1"
                >
